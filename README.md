...
| `DATABASE_URL` | 否 | - | 雲端資料庫連線字串（`postgres://`、`postgresql://` → PostgreSQL；`mysql://`、`mariadb://` → MySQL）。未設定時使用本機 SQLite（`DATABASE_PATH`）。`pg`/`mysql2` 為 optional dependencies，SQLite-only 部署不需安裝。 |
