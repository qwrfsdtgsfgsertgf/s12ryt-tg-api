...
describe("parseModelThinkingSuffix", () => {
  ...
  it("returns undefined level for invalid suffix word", () => {
    const result = parseModelThinkingSuffix("model(custom)");
    expect(result.model).toBe("model(custom)");
    expect(result.thinkingLevel).toBeUndefined();
  });
...
describe("extractThinkingLevel", () => {
  ...
  it("reverse-maps anthropic budget_tokens low (<10000)", () => {
    expect(
      extractThinkingLevel({ thinking: { type: "enabled", budget_tokens: 5000 } })
    ).toBe("low");
  });

  it("ignores disabled anthropic thinking", () => {
    expect(
...
describe("injectForGoogle", () => {
  ...
  it("preserves existing generationConfig", () => {
    ...
    expect((gc.thinkingConfig as Record<string, unknown>).thinkingBudget).toBe(
      GOOGLE_THINKING_BUDGET.medium
    );
  });
...
describe("Budget value ordering", () => {
  it("anthropic budgets are ordered high > medium > low", () => {
  ...
  it("google budgets are ordered high > medium > low", () => {
