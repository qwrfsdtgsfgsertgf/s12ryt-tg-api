/**
 * Unit tests for src/db/driver/mysqlDriver.ts
 *
 * Strategy:
 * - Requires a live MySQL instance via the TEST_MYSQL_URL env var (or
 *   TEST_DATABASE_URL when its scheme is mysql).
 * - When neither is present the whole suite is skipped, so local
 *   SQLite-only development never blocks. The CI workflow supplies the URL
 *   via a mysql service container.
...
const TEST_URL =
  ...
  (process.env.TEST_DATABASE_URL?.startsWith("mysql")
...
async function seedTable(driver: MysqlDriver, name: string): Promise<void> {
  await driver.exec(
    `CREATE TABLE ${name} (id INTEGER AUTO_INCREMENT PRIMARY KEY, name TEXT, flag INTEGER) ENGINE=InnoDB`,
  );
}

describeOrSkip("MysqlDriver", () => {
  let driver: MysqlDriver;

  beforeEach(async () => {
    driver = await MysqlDriver.create({
  ...
  describe("create / lifecycle", () => {
    it("initialises and reports the mysql dialect", async () => {
      expect(driver.dialect).toBe("mysql");
    });

    it("can run a trivial SELECT 1", async () => {
      const { rows } = await driver.query<{ one?: number }>(
        "SELECT 1 AS one",
      );
  ...
  describe("query", () => {
    ...
    it("returns matching rows with bound parameters", async () => {
      ...
      const { rows } = await driver.query<{ name: unknown }>(
        `SELECT name FROM ${t} WHERE flag = ? ORDER BY id`,
        [1],
      );
  ...
  describe("insert", () => {
    ...
    it("returns the generated AUTO_INCREMENT id", async () => {
    ...
    it("returns null when the statement is a no-op UPDATE", async () => {
      // A non-INSERT statement yields no insertId and thus no id.
  ...
  describe("exec", () => {
    ...
    it("runs multi-statement DDL split on semicolons", async () => {
      await driver.exec(
        `CREATE TABLE ${t1} (id INTEGER AUTO_INCREMENT PRIMARY KEY) ENGINE=InnoDB; CREATE TABLE ${t2} (id INTEGER AUTO_INCREMENT PRIMARY KEY) ENGINE=InnoDB;`,
      );
      // Probe each table with a no-row SELECT; missing tables would throw.
      await expect(driver.query(`SELECT * FROM ${t1} LIMIT 0`)).resolves
        .toBeDefined();
      await expect(driver.query(`SELECT * FROM ${t2} LIMIT 0`)).resolves
        .toBeDefined();
    });
  });
  ...
  describe("type coercion", () => {
    ...
    beforeEach(async () => {
      t = tableName("types");
      await driver.exec(
        `CREATE TABLE ${t} (id INTEGER AUTO_INCREMENT PRIMARY KEY, b INTEGER, big TEXT) ENGINE=InnoDB`,
      );
    });
    afterEach(async () => {
      await driver.exec(`DROP TABLE IF EXISTS ${t}`);
